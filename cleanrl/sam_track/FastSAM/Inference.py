...
def parse_args():
    ...
    parser.add_argument(
        "--model_path", type=str, default="cleanrl/sam_track/FastSAM/weights/FastSAM.pt", help="model"
    )
    parser.add_argument(
        "--img_path", type=str, default="cleanrl/sam_track/FastSAM/images/BeamRiderNoFrameskip-v4.png", help="path to image file"
    )
    ...
    parser.add_argument(
        "--output", type=str, default="cleanrl/sam_track/FastSAM/output/", help="image save path"
    )
    parser.add_argument(
...
def main(args):
    # load model
    t = time.time()
    model = FastSAM(args.model_path,verbose=False)
    ...
    a = everything_results[0].masks.data
    print(time.time()-t)
