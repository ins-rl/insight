...
from . import pretty_print, functions
...
DOMAIN = (-2, 2)    # Domain of dataset - range from which we sample x
# DOMAIN = np.array([[0, -1, -1], [1, 1, 1]])   # Use this format if each input variable has a different domain
N_TEST = 100        # Size of test dataset
DOMAIN_TEST = (-4, 4)   # Domain of test dataset - should be larger than training domain to test extrapolation
NOISE_SD = 0        # Standard deviation of noise for training dataset
# var_names = ["x", "y", "z"]
var_names = ["x"+str(i) for i in range(1,100000)]
...
class Benchmark:
    ...
    def __init__(self, results_dir, n_layers=2, reg_weight=5e-3, learning_rate=1e-2,
        ...
        self.activation_funcs = [
            ...
            *[functions.Identity()] * 4*n_funcs,
            *[functions.Square()] * 4*n_funcs,
            *[functions.Sin()] * 2*n_funcs,
            *[functions.Exp()] * 2*n_funcs,
            *[functions.Sigmoid()] * 2*n_funcs,
            *[functions.Invx()] * 2*n_funcs,
    ...
    def benchmark(self, func, func_name, trials):
        ...
        print(len(error_test_sorted),len(expr_list_sorted),trials)
        ...
        for trial in range(trials):
            ...
            net = SymbolicNet(self.n_layers,
                              ...
                              initial_weights=[
                                  ...
                                  torch.fmod(torch.normal(0, init_sd_middle, size=(width, width + n_double)), 2),
                                  torch.fmod(torch.normal(0, init_sd_last, size=(width, y.shape[1])), 2)
                              ]).to(device)
...
if __name__ == "__main__":
    ...
    # bench.benchmark(lambda x, y: torch.divide(x, (y+10)), func_name="x/y", trials=1)
    # bench.benchmark(lambda x, y: (x+y>0)+0.0, func_name="x+y>0", trials=20)
